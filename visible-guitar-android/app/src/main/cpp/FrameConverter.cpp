...
FrameConverter::FrameConverter(const std::vector<cv::Point>& points) {
    this->points = points;
}

cv::Mat FrameConverter::create_mask(cv::Mat &input) {
...
cv::Mat FrameConverter::convert_to_grayscale_canny_with_dilate(cv::Mat& input) {
...
void FrameConverter::rotate_to_horizontal_strings(cv::Mat& input) {

}

void FrameConverter::convert(cv::Mat& input, cv::Mat& output) {
